...
pnpm monorepo. Two independent TanStack Start apps, one shared DB package.

```
apps/
  dashboard/        # Agency dashboard — app.portava.io (port 3001)
  portal/           # Client portal — {agency}.portava.io (port 3002)
packages/
  db/               # @portava/db — Kysely client + TypeScript DB types
db/
  schema.sql        # Postgres DDL (source of truth)
  seed.sql          # Dev seed data (1 agency, 3 portals, files, events)
docker-compose.yml  # Postgres 18 on port 5433
justfile            # Dev task runner
biome.json          # Formatter + linter (Biome v2)
tsconfig.base.json  # Shared TS config extended by all packages
```

Each TanStack Start app has:
```
src/
  router.tsx           # getRouter() — registers routeTree
  routes/
    __root.tsx         # Shell: <html>, <HeadContent />, <Scripts />
    index.tsx          # Entry route
  routeTree.gen.ts     # Auto-generated by TanStack Router (do not edit)
vite.config.ts         # tanstackStart + nitro + viteReact plugins
tsconfig.json          # Extends ../../tsconfig.base.json, adds ~/src alias
```
...
# 3. Seed dev data (first time, or after schema changes)
just db-reset-seed

# 4. Run apps
just dev-dashboard    # http://localhost:3001
just dev-portal       # http://localhost:3002
```
...
| Command            | Description                              |
|--------------------|------------------------------------------|
| `just dev-dashboard` | Start agency dashboard dev server      |
| `just dev-portal`    | Start client portal dev server         |
| `just db-start`      | Start Postgres container               |
| `just db-stop`       | Stop Postgres container                |
| `just db-psql`       | Open psql shell                        |
| `just db-reset`      | Drop + recreate DB from schema.sql     |
| `just db-seed`       | Insert seed data                       |
| `just db-reset-seed` | Reset + seed in one command            |

## Database

- Postgres 18 via Docker, port **5433**
- Connection string: `postgresql://portava:portava@localhost:5433/portava`
- Schema: `db/schema.sql` — all DDL lives here, IDs use `uuidv7()`
- Kysely types: `packages/db/src/index.ts` — mirrors the schema exactly
- `@portava/db` is a workspace package imported by both apps

## Tooling

- **Biome v2** — formatter + linter. Format on save via Claude Code hook (`biome check --write`). Run manually: `pnpm format` / `pnpm lint`
- **TypeScript 6** — strict mode, `moduleResolution: bundler`, `~/` path alias in each app
